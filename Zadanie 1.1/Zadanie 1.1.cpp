...
int main()
{
	...
	взятое со знаком + если угол между вектором AB и осью острый и со знаком - если угол тупой.
	Также можно обойтись и без угла. Достаточно только скалярное произведение разделить на длину вектора оси.*/
	cout << "\n"; //Пропускаю строчку в консоли для наглядности.
