...
вектор: каждая компонента вектора – это частное от деления числа на её
...
public:
	...
	//Перегрузка унарного оператора ! (логическое отрицание, НЕ), который будет выдавать проекцию на ось абсцисс.
	double operator!()
	{//Проекция вектора на ось абсцисс это просто его координата X.
		double res = coordinateX;//Вычисление проекции.
		return res;
	}
	//Перегрузка унарного оператора ~ (побитовая инверсия), который будет выдавать проекцию на ось абсцисс.
	double operator~()
	{//Проекция вектора на ось ординат это просто его координата Y.
		double res = coordinateY;//Вычисление проекции.
		return res;
	}
	//Перегрузка бинарного оператора && (Логическое умножение, И), который будет выдавать проекцию одного вектора на другой.
...
int main()
{	//Чтобы не возникло проблем с работой вещественных и целочисленных данных, наш вектор теперь будет не целосисленный, а вещественный.
	...
	C.print();
	cout << "\n"; //Пропускаю строчку в консоли для наглядности.
	//Проекция вектора на ось абсцисс.
	...
	cout << "\n\n\nPerehodim k deleniyu veshchestvennykh chisel na vektor."; //Пропускаю строчку в консоли для наглядности
	...
	cout << "\n\n\n"; //Пропускаю строчи в консоли для наглядности
